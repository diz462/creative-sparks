...
%% Filter to remove small pixel areas/volumes

% Method 1 = bone_pores
% Method 2 = ima_mask_adjusted2

ii = 17; %  image number
pixel_volume = 8; % filter volume size

bone_pores_filt = bwareaopen(bone_pores,pixel_volume,26);
bone_pores_filt = double(bone_pores_filt);

ima_mask_adjusted2_filt = bwareaopen(ima_mask_adjusted2,pixel_volume,26);
ima_mask_adjusted2_filt = double(ima_mask_adjusted2_filt);

diff_bone_pore = abs(bone_pores-bone_pores_filt);
diff_image_mask_adjusted = abs(ima_mask_adjusted2-ima_mask_adjusted2_filt);

% 2D filter on single image
pixel_area = 4;
bone_pores_filt2D = bwareaopen(bone_pores(:,:,ii),pixel_area,8);

%2D vs 3D Comparison
figure 
tiled = tiledlayout(2,3,'TileSpacing','none','Padding','compact');
title(tiled,"Method 1 Image "+rand_ima(ii),"Pixel Area Filter = "+pixel_area+ ...
    "  Pixel Volume Filter = "+pixel_volume);

nexttile(1)
imshowpair(ima_mask_gray(:,:,ii),bone_pores_filt2D);
axis image;
title("2D Filter");
...
nexttile(5)
imshowpair(bone_pores_filt2D,bone_pores_filt(:,:,ii));
axis image;
title("2D Filter vs 3D Filter",'Units','normalized','Position',[0.18 0.05] ...
    ,'Color','g','FontSize',8);
